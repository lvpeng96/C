...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < 100; i++)
            {
                comboBox1.Items.Add(i.ToString() + "  时");  //此处为 空格秒，为了在后边提取时，
            ...
            for (i = 0; i < 100; i++)
            {
                comboBox2.Items.Add(i.ToString() + "  分");  //此处为 空格秒，为了在后边提取时，
                //若为个位数时，下面的函数 从0位提取，提取两位会提取到汉字 秒，会报错
                // string data = str.Substring(0, 2); 
            }
            label7.Text = "0 分";
            ...
            for (i = 1; i < 100; i++)
            {
                comboBox3.Items.Add(i.ToString() + "  秒");  //此处为 空格秒，为了在后边提取时，
                //若为个位数时，下面的函数 从0位提取，提取两位会提取到汉字 秒，会报错
                // string data = str.Substring(0, 2); 
            }
            label3.Text = "0  秒";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            testbum = hourstr.Substring(0, 2);
            gettime = Convert.ToInt16(testbum);    //将字符转换成整形    //提取的是字符，汉字也为一个字符字母也是一个字符数字也是一个字符
